...
requestData::~requestData()
{
    cout << "destruction:~requestData()" << endl;
...
void requestData::handleRequest()
{
    ...
    while (true)
    {
        ...
        if (state == STATE_RECV_BODY)
        {
            ...
            /**
             * 有个问题 ， 第二次读的content加在第一次的内容后边，进行substr时会发生错误
             * 所以，这个if条件需要调整 
             */
            if (content.size() < content_length)    //一次没发送完，继续读
                continue;
            state = STATE_ANALYSIS;
        }
...
int requestData::parse_URI()
{
    ...
    // 去掉请求行所占的空间，节省空间
    string request_line = str.substr(0, pos);   //请求行
    // 这里应该没有去掉换行符吧  Question 1
    if (str.size() > pos + 1)                   //这是否有必要
...
{
    //引用 请求行以分析完并已经在content中去掉
    string &str = content;
    int key_start = -1, key_end = -1, value_start = -1, value_end = -1;
    int now_read_line_begin = 0;    //行开始 str的index
    bool notFinish = true;
...
//        fclose(fp);
    ...
    else if (method == METHOD_GET)
    {
        ...
        int dot_pos = file_name.find('.');  //文件扩展符
        const char* filetype;
        if (dot_pos < 0) 
        ...
        struct stat sbuf;
        if (stat(file_name.c_str(), &sbuf) < 0) // 成功返回 0 
        {
...
{
    /* 先发送头 后发送内容，说明write函数可以分批次使用 */
    short_msg = " " + short_msg;
    char send_buff[MAX_BUFF];
    string body_buff, header_buff;
    body_buff += "<html><title>404 file not found</title>";
    body_buff += "<body bgcolor=\"ffffff\">";
    body_buff += to_string(err_num) + short_msg;
    body_buff += "<hr><em> AlbertLin's Web Server JN university</em>\n</body></html>";
